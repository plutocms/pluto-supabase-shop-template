  ...
  <div>
    <header class="bg-slate-400 dark:bg-slate-950 py-20 px-4">
      <div class="text-center space-y-6">
        ...
        <h1 class="text-4xl font-bold text-center text-white">Pluto Shop</h1>
    ...
    <main>
      <Container class="py-8 max-w-full">
        <section class="py-8 flex flex-col gap-y-6">
          ...
          <div v-if="!products?.length" class="text-center text-gray-500 py-16">
          ...
          >
            <article
              ...
              :key="product.id"
              class="group/product relative border border-gray-300/10 rounded-2xl p-2 sm:p-3 flex flex-col gap-y-2 transition-all hover:bg-white/5 dark:hover:bg-slate-800/50 hover:border-white/20"
            >
              <!-- Product Link -->
              <NuxtLink
            ...
            <div
              v-for="n in Math.max(0, 5 - (products?.length || 0))"
              :key="'placeholder-' + n"
